...
def make_plot():
    ...
    ssh.connect(hostname='genesforgood-banichlab.rhcloud.com', username='54bf3ae6e0b8cd98c6000041', key_filename='../../../data/id_rsa')
    ...
    trial_data = [part['data'] for part in all_data if part['hitId'] == '3SBNLSTU6UOUH81LE2VKPC2CNG1DZU']
    good_trials = [trial for trial in trial_data if 'input_words' in trial[-1]['trialdata'].keys()]

    
    qs_data = []
    for sub_data in all_data:
        try:
            qs = sub_data['questiondata']
            try:
                qs['uniqueid'] = sub_data['data'][0]['uniqueid']
            except IndexError:
                qs['uniqueid'] = 'NA'

            if sub_data['hitId'] == '3SBNLSTU6UOUH81LE2VKPC2CNG1DZU':
                qs_data.append(qs)
        except ValueError:
            pass

    qs_data_id = pd.DataFrame(qs_data).dropna().drop('openended', 1)
    qs_data_id[['difficulty', 'engagement', 'informative']] = qs_data_id[['difficulty', 'engagement', 'informative']].astype('float')
    qs_data= qs_data_id[['difficulty', 'engagement', 'informative']]

    tab1 = Panel(child=BoxPlot(qs_data, title="Questionnaires", width=400, height=400), title="questionnaire answers")


    good_data = pd.concat([pd.io.json.json_normalize(trial) for trial in good_trials])
    good_data = good_data.rename(columns={'trialdata.input_words' :'input_words', 'trialdata.target_words':'target_words', 'trialdata.block':'block'})
    kt_data = good_data[pd.isnull(good_data.input_words) == False][['input_words', 'target_words', 'uniqueid', 'block']].reset_index(drop=True)
    kt_data['perc_score'] = kt_data.apply(lambda x: np.array([1 if target in x['target_words'] else 0 for target in x['input_words']]).sum() / (len(x['target_words']) * 1.0), axis=1)
...
def view():
    if not session.get('logged_in'):
    	...
    	fig = file_html(make_plot(), CDN, "dashboard")
